"""Custom sampling function with contrastive decoding support."""
...
def sample(
    ...
    **model_kwargs,
    ...
    """Override the default sampling routine to support contrastive decoding."""
...
def enable_shield_sampling():
    """Inject the custom sampling function into ``GenerationMixin``."""
...
def greedy_search(
    ...
    **model_kwargs,
    ...
    """Override default greedy_search to support contrastive decoding and multi-GPU devices."""
    logits_processor = logits_processor if logits_processor is not None else LogitsProcessorList()
    stopping_criteria = stopping_criteria if stopping_criteria is not None else StoppingCriteriaList()
    if max_length is not None:
        warnings.warn(
    ...
    while True:
        ...
        if use_cd:
            ...
            next_token_scores = cd_logits
            next_tokens = torch.argmax(cd_logits, dim=-1).to(unfinished_sequences.device)
        else:
            next_token_scores = logits_processor(input_ids, next_token_logits)
            next_tokens = torch.argmax(next_token_scores, dim=-1).to(unfinished_sequences.device)
