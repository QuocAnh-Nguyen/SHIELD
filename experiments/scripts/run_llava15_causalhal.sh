...
CONFIG_FILE="$REPOSITORY_ROOT/experiments/configs/llava15_causalhal.env"
DRY_RUN=0
...
answers_file="$OUTPUT_DIR/llava15_causalhal_answers_seed${SEED}.json"
...
inference_command=(
    python experiments/eval/causalhal_llava.py
    --model-path "$MODEL_PATH"
    --question-file "$CAUSALHAL_QA_FILE"
    --image-folder "$CAUSALHAL_IMAGE_DIR"
