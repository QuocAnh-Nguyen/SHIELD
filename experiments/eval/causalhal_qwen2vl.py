...
def eval_model(args):
    ...
    missing_images = [
        q["image_name"] for q in questions
        if not os.path.exists(os.path.join(args.image_folder, q["image_name"]))
    ]
    if missing_images:
    ...
    missing_captions = [
        q["image_name"] for q in questions
        if find_text_by_image(q["image_name"], captions) is None
    ]
    if missing_captions:
    ...
    for line in tqdm(questions):
        ...
        if not os.path.exists(image_path):
            raise FileNotFoundError(f"Missing image file for id {line['id']}: {image_path}")
        ...
        shield_kw = model.shield_prepare(image, inputs.pixel_values[0], image_name, image_grid_thw, use_cd=args.use_cd)
        ...
        results.append({"image_name": image_name, "type": line["type"], "answer": answer, "tag": line["tag"], "id": line["id"]})
